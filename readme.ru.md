...
- 4. [РАСШИРЕНИЕ PHPMUSSEL](#user-content-SECTION4)
- 5. [НАСТРОЙКИ](#user-content-SECTION5)
- 6. [ФОРМАТ СИГНАТУРЕЙ](#user-content-SECTION6)
...
Кроме того, если не хотите использовать Composer, вы можете скачать предварительно упакованные ZIP-файлы отсюда:
...
Если хотите использовать фронтенд phpMussel, вы можете настроить все на странице конфигурации фронтэнда. Но, начиная с v3, информация о входе в фронтенд сохраняется в вашем файле конфигурации, поэтому, чтобы войти в фронтенд, вам нужно как минимум настроить аккаунт для входа в фронтенду, а затем оттуда вы сможете войти и использовать страницу конфигурации фронтэнда для настройки всего остального.
...
Необязательно, phpMussel может попытаться восстановить имена загрузок, если что-то не так, если хотите:
...
![Скриншот](https://raw.githubusercontent.com/phpMussel/extras/master/screenshots/cli-v3.5.0.png)
...
*Смотрите также: [Как получить доступ к конкретным сведениям о файлах при их сканировании?](#user-content-SCAN_DEBUGGING)*
...
Полезный совет: Вы можете прикрепить информацию о дате и времени к именам файлов журналов, используя заполнители формата времени. Доступные заполнители формата времени отображаются на <a onclick="javascript:toggleconfigNav('coreRow','coreShowLink')" href="#config_core_time_format">`core➡time_format`</a>.
...
##### «disabled_channels» `[string]`
- Это можно использовать для предотвращения использования phpMussel определенных каналов при отправке запросов.
...
##### «entropy_limit» `[float]`
- Предел энтропии для сигнатурей, использующих нормализованные данные (стандарт 7.7). В данном контексте, энтропия определяется как энтропия шеннона содержимого сканируемого файла. При оба превышении предела энтропии и предела размера файла энтропии, чтобы снизить риск ложноположительный, некоторые сигнатуры, использующие нормализованные данные, будут игнорироваться.

##### «entropy_filesize_limit» `[string]`
- Предел размера файла энтропии для сигнатурей, использующих нормализованные данные (стандарт 256КБ). При оба превышении предела энтропии и предела размера файла энтропии, чтобы снизить риск ложноположительный, некоторые сигнатуры, использующие нормализованные данные, будут игнорироваться.

#### «quarantine» (Категория)
Конфигурация для карантина.

##### «quarantine_key» `[string]`
- phpMussel может изолировать загруженные файлы на карантин, если это будет Вам нужно. Эту функцию должны деактивировать те пользователи, которые хотят лишь защитить свои интернет-сайты или окружение своего хоста, но дальнейший контроль выделенных файлов им не интересен. Активировать эту функцию должны те пользователи, которые хотят анализировать изолированные файлы на вредоносность или тому подобное. Изоляция выделенных файлов иногда может использоваться для поиска ошибки при частом появлении ложноположительный. Чтобы деактивировать функцию карантина, не заполняйте `quarantine_key`, и удалите содержание строки, если она заполнена. Для активирования функции карантина задайте значение. `quarantine_key` = Это важный элемент карантинной функции, способная защитить функцию карантина от атак вредоносных программ, и предотвратить запуск удерживаемых на карантине файлов. Значение `quarantine_key` Вы должны хранить втайне от всех, как и пароли. Оптимально в сочетании с `delete_on_sight`.
...
```
vt_suspicion_level
├─0 (Сканировать только файлы с эвристическим весом.): Сканироваться будут только файлы с
│ эвристическим весом. Эвристический вес
│ может быть получен из-за сигнатур,
│ предназначенных для обнаружения общих
│ отпечатков пальцев, связанных с
│ потенциальным заражением, которые не
│ обязательно гарантируют заражение. Поиск
│ в таких случаях может служить для
│ получения второго мнения о результатах,
│ которые оправдывают подозрения, но в
│ остальном не дают никакой уверенности.
├─1 (Сканировать файлы с эвристическим весом, исполняемые файлы, и файлы потенциально содержащие исполняемые данные.): Примеры исполняемых файлов, и файлов
│ потенциально содержащих исполняемые
│ данные, включают файлы Windows PE, файлы Linux ELF,
│ файлы Mach-O, файлы DOCX, файлы ZIP, и т.д.
...
##### «prefix» `[string]`
- Указанное здесь значение будет добавлено в начало всех ключей записи кэша. Стандарт = «phpMussel_». Когда на одном сервере существует несколько установок, это может быть полезно для хранения их кэшей отдельно.
...
##### «forbid_on_block» `[bool]`
- Отправлять заголовок HTTP 403 при отображении сообщения о блокировке загрузки или придерживаться обычного HTTP 200? False = Нет (200); True = Да (403) [Стандарт].
...
##### «theme_mode» `[string]`
- Режим для тема, которая будет использоваться для событий блокировки.
...
##### «custom_header» `[string]`
- Вставляется как HTML в начало всех страниц «загрузка отказана». Это может быть полезно, если хотите включить логотип веб-сайта, персонализированный заголовок, скрипты, или подобное на всех таких страницах.

##### «custom_footer» `[string]`
- Вставляется как HTML в конце всех страниц «загрузка отказана». Это может быть полезно, если хотите включить юридическое уведомление, контактную ссылку, деловую информацию, или подобное на всех таких страницах.

#### «phpmailer» (Категория)
Конфигурация для PHPMailer (используется для двухфакторной аутентификации и для уведомлений по электронной почте).
...
##### «enable_notifications» `[string]`
- Если хотите получать уведомления по электронной почте, когда загрузка заблокирована, укажите адрес электронной почты получателя здесь.
...
- [Я столкнулся с проблемой при использовании phpMussel, и я не знаю, что с этим делать! Пожалуйста помоги!](#user-content-ENCOUNTERED_PROBLEM_WHAT_TO_DO)
- [Я хочу использовать phpMussel v3 с версией PHP ниже 7.2.0; Вы можете помочь?](#user-content-MINIMUM_PHP_VERSION_V3)
- [Могу ли я использовать одну установку phpMussel для защиты нескольких доменов?](#user-content-PROTECT_MULTIPLE_DOMAINS)
...
- [Как получить доступ к конкретным сведениям о файлах при их сканировании?](#user-content-SCAN_DEBUGGING)
- [Черные списки – Белые списки – Серые списки – Каковы они и как их использовать?](#user-content-BLACK_WHITE_GREY)
- [Что такое «PDO DSN»? Как я могу использовать PDO с phpMussel?](#user-content-HOW_TO_USE_PDO)
- [Моя функция загрузки асинхронна (например, использует ajax, ajaj, json и т.д.). Я не вижу специальных сообщений или предупреждений, когда загрузка заблокирована. В чем дело?](#user-content-AJAX_AJAJ_JSON)
- [Может ли phpMussel обнаружить EICAR?](#user-content-DETECT_EICAR)
...
Нет. PHP≥7.2 является минимальным требованием для phpMussel v3.
...
Последнее обновление: 13 Мая 2026 г (2026.05.13).
