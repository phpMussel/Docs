...
##### 『entropy_limit』 `[float]`
- 使用規範化資料的簽名的熵極限（默認為7.7）。 在此上下文中，熵被定義為被掃描檔案內容的香農熵。 當熵限制和熵檔案大小限制都超出時，為了降低假陽性的風險，一些使用標準化資料的簽名將被忽略。

##### 『entropy_filesize_limit』 `[string]`
- 使用規範化資料的簽名的熵檔案大小限制（默認為256KB）。 當熵限制和熵檔案大小限制都超出時，為了降低假陽性的風險，一些使用標準化資料的簽名將被忽略。
