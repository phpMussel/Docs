...
##### 『entropy_limit』 `[float]`
- 使用规范化数据的签名的熵限制（默认为7.7）。​在此上下文中，熵被定义为被扫描文件内容的香农熵。​当熵限制和熵文件大小限制都超出时，为了降低误报的风险，一些使用标准化数据的签名将被忽略。

##### 『entropy_filesize_limit』 `[string]`
- 使用規範化資料的簽名的熵檔案大小限制（默認為256KB）。​當熵限制和熵檔案大小限制都超出時，為了降低誤報的風險，一些使用標準化資料的簽章將被忽略。
